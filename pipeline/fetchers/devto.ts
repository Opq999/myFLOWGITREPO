...
export async function fetchDevto(opts: { backfill: boolean }): Promise<Candidate[]> {
  const results: Candidate[] = [];
  for (const tag of DEVTO_TAGS) {
    const top = opts.backfill ? 365 : 2;
    const url = `https://dev.to/api/articles?tag=${tag}&top=${top}&per_page=${opts.backfill ? 30 : 10}`;
